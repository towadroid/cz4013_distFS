...
private:
    ...
    void
    exec_service(std::integral_constant<Service_type, Service_type::not_a_service> x, const UdpServer_linux &server,
                 unsigned char *message);

    void exec_service(std::integral_constant<Service_type, Service_type::service1> x, const UdpServer_linux &server,
                      unsigned char *message);

    void
    exec_service(std::integral_constant<Service_type, Service_type::register_client> x, const UdpServer_linux &server,
                 unsigned char *message);

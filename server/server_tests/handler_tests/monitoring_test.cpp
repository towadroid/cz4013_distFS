...
TEST(Monitor, remove_content) {
    ...
    BytePtr inc_msg1;
    unsigned int inc1_len = utils::pack(inc_msg1, requestID, raw1_len, 0, raw1_len, raw1.get()); //remove all content

    BytePtr raw2;
    unsigned int raw2_len = utils::pack(raw2, 3, std::string("test_file"), 600000); //monitor for 600s
...
TEST(Monitor, expired) {
    ...
    BytePtr raw2;
    unsigned int raw2_len = utils::pack(raw2, 3, std::string("test_file"), 950); // ask to monitor for 950 ms
...
TEST(Monitor, different_file) {
    MockHandler mock_handler;
    MockUdpServer_linux mock_server;
    prepare_file();
    prepare_file("test_file2");
    ...
    BytePtr raw2;
    unsigned int raw2_len = utils::pack(raw2, 3, std::string("test_file2"), 10000); // ask to monitor for 10s
